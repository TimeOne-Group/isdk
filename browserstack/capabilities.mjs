...
export async function fetchCapabilities() {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch browserstack capabilities: ${response.status} ${response.statusText}`,
    );
  }
...
function getOs(capabilities) {
  const desktop = [
    ...new Set(
      capabilities.filter(({ device }) => !device).map(({ os }) => os),
    ),
  ];
  const mobile = [
    ...new Set(
...
function builBrowserCapabilities({ allCapabilities, name, osList }) {
  ...
  return generatedBrowserCapabilities.filter(
    ({ os, os_version, browser, browser_version }) => {
      const existingCapacities = allCapabilities.filter(
        (capability) =>
          capability.os === os &&
          capability.os_version === os_version &&
...
function filterAvailableMobileCapabilities(
  mobileCapabilities,
  allCapabilities,
) {
  return mobileCapabilities.filter(({ device, os_version }) => {
    const isAvailable = allCapabilities.some(
      (capability) =>
        ...
        capability.os_version === String(os_version),
    );

    if (!isAvailable) {
      console.warn(
        `BrowserStack device not available, skipped: ${device} (os ${os_version})`,
      );
    }
...
export default async function getCapabilities(name) {
  ...
  const browserCapabilities = builBrowserCapabilities({
    allCapabilities,
    name,
    osList: desktop,
  });
  const mobileCapabilities = buildMobileCapabilities(
    [...ios, ...android],
    name,
  );
  const availableMobileCapabilities = filterAvailableMobileCapabilities(
    mobileCapabilities,
    allCapabilities,
  );
